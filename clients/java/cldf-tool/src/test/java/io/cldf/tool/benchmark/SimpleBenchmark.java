...
public class SimpleBenchmark {
    ...
    public Map<String, Object> benchmarkInMemoryGradePyramid() {
        Map<String, Long> gradeCounts = archive.getClimbs().stream()
            .filter(c -> c.getGrades() != null && c.getGrades().getGrade() != null)
            .collect(Collectors.groupingBy(
                c -> c.getGrades().getGrade(),
                Collectors.counting()
            ));
        
        List<Map<String, Object>> results = gradeCounts.entrySet().stream()
        ...
        for (int i = 0; i < size; i++) {
            ...
            climbs.add(Climb.builder()
                ...
                .grades(Climb.GradeInfo.builder()
                    ...
                    .build())
                .finishType(random.nextBoolean() ? "redpoint" : "onsight")
                .build());
        }
