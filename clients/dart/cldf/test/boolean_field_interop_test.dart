...
void main() {
  group('Boolean Field Serialization Interoperability', () {
    test('Dart-generated archive with isRepeat and isIndoor should be readable by Java', () async {
      ...
      if (File(cliPath).existsSync()) {
        ...
        // Query sessions
        final sessionsResult = await Process.run(
          cliPath,
          ['query', outputPath, '--select', 'sessions', '--json', 'json'],
          workingDirectory: Directory.current.path,
        );

        if (sessionsResult.exitCode != 0) {
          print('Java CLI query failed: ${sessionsResult.stderr}');
          fail('Java CLI could not read Dart-generated archive with boolean fields');
        }

        final sessionsOutput = jsonDecode(sessionsResult.stdout as String) as Map<String, dynamic>;
        final sessions = (sessionsOutput['data'] as Map<String, dynamic>)['results'] as List;
        expect(sessions.length, equals(1));
        final session = sessions.first as Map<String, dynamic>;
        expect(session.containsKey('isIndoor'), isTrue, reason: 'Session JSON should contain isIndoor field');
        expect(session['isIndoor'], isTrue, reason: 'Session isIndoor should be true');
        print('✓ Java correctly read Session.isIndoor = true');

        // Query climbs
        final climbsResult = await Process.run(
          cliPath,
          ['query', outputPath, '--select', 'climbs', '--json', 'json'],
          workingDirectory: Directory.current.path,
        );

        final climbsOutput = jsonDecode(climbsResult.stdout as String) as Map<String, dynamic>;
        final climbs = (climbsOutput['data'] as Map<String, dynamic>)['results'] as List;
        expect(climbs.length, equals(1));
        final climb = climbs.first as Map<String, dynamic>;
        expect(climb.containsKey('isRepeat'), isTrue, reason: 'Climb JSON should contain isRepeat field');
        expect(climb['isRepeat'], isTrue, reason: 'Climb isRepeat should be true');
        expect(climb.containsKey('isIndoor'), isTrue, reason: 'Climb JSON should contain isIndoor field');
        expect(climb['isIndoor'], isTrue, reason: 'Climb isIndoor should be true');
        print('✓ Java correctly read Climb.isRepeat = true');
        print('✓ Java correctly read Climb.isIndoor = true');

        // Query locations
        final locationsResult = await Process.run(
          cliPath,
          ['query', outputPath, '--select', 'locations', '--json', 'json'],
          workingDirectory: Directory.current.path,
        );

        final locationsOutput = jsonDecode(locationsResult.stdout as String) as Map<String, dynamic>;
        final locations = (locationsOutput['data'] as Map<String, dynamic>)['results'] as List;
        expect(locations.length, equals(1));
        final location = locations.first as Map<String, dynamic>;
        expect(location.containsKey('isIndoor'), isTrue, reason: 'Location JSON should contain isIndoor field');
        expect(location['isIndoor'], isTrue, reason: 'Location isIndoor should be true');
        ...
        print('Java CLI not available at $cliPath, skipping Java verification');
        print('Note: Run this test after building the Java CLI to verify full interoperability');
      }
    ...
    test('Java-generated archive with isRepeat and isIndoor should be readable by Dart', () async {
      // Check if Java CLI is available
      final cliPath =
          '/Users/petrmacek/git-mirrors/crushlog-data-format-spec/clients/java/cldf-tool/build/native/nativeCompile/cldf';

      if (!File(cliPath).existsSync()) {
        print('Java CLI not available, skipping Java->Dart interoperability test');
